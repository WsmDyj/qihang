...
const removeLike = async (article_id) => {
  const sqlArticle = `update blogs set likeCount = likeCount - 1 where article_id='${article_id}' `
  const sql = `delete from likes where article_id='${article_id}';`
  const updataData = await exec(sqlArticle)
...
const getLikelists = async (author) => {
  const sql = `SELECT blogs.article_id,author,title,content,createtime,likeCount,markdown FROM blogs, likes where likes.like_author = '${author}' and likes.article_id = blogs.article_id order by likes.id desc;`
